    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Your weak areas and a personalised focus plan.</p>
        </div>
      ...
      {!loading && rec && (
        <>
          ...
          {rec.weakSubjects?.length > 0 && (
            <section>
              <h2 className="mb-2 text-sm font-bold uppercase tracking-wide text-slate-500">Focus subjects</h2>
              <div className="grid gap-3 sm:grid-cols-2">
                {rec.weakSubjects.map((w, i) => (
                  <div key={i} className="card p-4">
                    <div className="flex items-center justify-between">
                      <h3 className="font-semibold">{w.subject}</h3>
                      <span className="badge bg-amber-100 text-amber-800">Priority {w.priority}</span>
                    </div>
          ...
          {rec.weakTopics?.length > 0 && (
            <section>
              <h2 className="mb-2 text-sm font-bold uppercase tracking-wide text-slate-500">Topics to revise</h2>
          ...
          {rec.focusPlan?.length > 0 && (
            <section>
              <h2 className="mb-2 text-sm font-bold uppercase tracking-wide text-slate-500">Your focus plan</h2>
          ...
          {perTopic.length > 0 && (
            <section>
              <h2 className="mb-2 text-sm font-bold uppercase tracking-wide text-slate-500">Performance by topic</h2>
              <div className="card divide-y divide-slate-100">
                {perTopic.map((p, i) => (
                  <div key={i} className="flex items-center gap-3 p-3">
                    <div className="flex-1 text-sm">
                      <span className="font-medium">{p.subject}</span> · {p.topic}
                      <span className="ml-2 text-xs text-slate-400">{p.attempts} attempt(s)</span>
                    </div>
